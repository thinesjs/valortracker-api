...
class RiotAPIController extends Controller
{
    ...
    public function getMatchDetails(Request $request): JsonResponse
    {
        ...
        if(!empty($matchDetails) && !isset($matchDetails->errorCode)) return response()->json($matchDetails, 200); else return response()->json(['status' => 'error', 'data' => 'invalid access token'], 401);
    }
